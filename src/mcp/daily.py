...
async def build_daily_summary(
    profile: Profile, domaines: list[str] | None = None
) -> str:
    ...
    for domain in active_domains[:_MAX_DOMAINS]:
        ...
        top = filter_and_rank(
            domain_articles, profile, domains=[domain], limit=_TOP_PER_DOMAIN
        )
        if not top:
            continue
        ...
        for article in top:
            ...
            lines.append(
                f"- **[{article.title}]({article.url})** · {article.source} · {pub}"
            )
            if article.summary:
    ...
    if total == 0:
        lines.append(
            "Aucun article disponible pour aujourd'hui. Relancez dans quelques instants pour récupérer les dernières actualités."
        )
    else:
        lines.append(
